        ...
        [
            {
                ...
                "label": ", ".join(
                    edge["node"]["name"]
                    for edge in pr["node"]["pullRequest"]["labels"]["edges"]
                ),
                "commits": pr["node"]["pullRequest"]["commits"]["totalCount"],
                "merged_by_id": (
                    pr["node"]["pullRequest"]["mergedBy"]["id"]
                    if pr["node"]["pullRequest"]["merged"]
                    and pr["node"]["pullRequest"]["mergedBy"]
                    and "id" in pr["node"]["pullRequest"]["mergedBy"]
                    else ""
                ),
            ...
            if not (
                pr["node"]["pullRequest"]["repository"]["isArchived"]
                or pr["node"]["pullRequest"]["state"] == "CLOSED"
            )
            if not (
                "invalid"
                in [
