...
def convert_to_intermediate_table(data):
    ...
    review_contribution_list = df['data.user.contributionsCollection.pullRequestReviewContributions.edges'][0]
    pr_contribution_list = df['data.user.contributionsCollection.pullRequestContributions.edges'][0]
    issue_contribution_list = df['data.user.contributionsCollection.issueContributions.edges'][0]
    repo_contribution_list = df['data.user.contributionsCollection.repositoryContributions.edges'][0]

    new_df = pd.DataFrame(columns=['github_id', 'type', 'repo_id', 'repo_owner_id', 'pr_status', 'label','commits', 'review_type', 'forks', 'stars', 'comments', 'reactions', 'merged_by_id', 'author_id', 'is_fork', 'created_at', 'last_updated_at'])

    new_df = format_issue_comments(issue_comment_list, user_github_id, user_name, new_df)
    new_df = format_pr_review_contributions(review_contribution_list, user_github_id, user_name, new_df)
...
def format_pr_review_contributions(review_contribution_list, user_id, user_name, df):
    for pr_review in review_contribution_list:
        ...
        df = df.append({'user_id': user_id, 'user_name': user_name, 'type': 'PR Reviewed',
                    ...
                    'reactions': reactions, 'created_at': created_at,
                    'last_updated_at': last_updated_at}, ignore_index=True )
    return(df)
