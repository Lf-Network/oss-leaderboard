...
def convert_to_intermediate_table(data: str) -> pd.DataFrame:
    ...
    return {
        "intermediate_table": new_df,
        "page_info": extract_page_info(df)
    }


def extract_page_info(df: pd.DataFrame) -> Dict:
    has_next_page_T1 = df["data.user.contributionsCollection.pullRequestContributions.pageInfo.hasNextPage"][0]
    end_cursor_T1 = df["data.user.contributionsCollection.pullRequestContributions.pageInfo.endCursor"][0]

    has_next_page_T2 = df["data.user.contributionsCollection.pullRequestReviewContributions.pageInfo.hasNextPage"][0]
    end_cursor_T2 = df["data.user.contributionsCollection.pullRequestReviewContributions.pageInfo.endCursor"][0]

    has_next_page_T3 = df["data.user.contributionsCollection.issueContributions.pageInfo.hasNextPage"][0]
    end_cursor_T3 = df["data.user.contributionsCollection.issueContributions.pageInfo.endCursor"][0]
    ...
    has_next_page_T5 = df["data.user.contributionsCollection.repositoryContributions.pageInfo.hasNextPage"][0]
    end_cursor_T5 = df["data.user.contributionsCollection.repositoryContributions.pageInfo.endCursor"][0]
